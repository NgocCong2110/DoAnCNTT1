...
export class TrangLichSuUngTuyen implements OnInit {
  ...
  huyUngTuyen(viec: any) {
    const thong_tin = {
      ma_nguoi_tim_viec: this.thongTin?.thong_tin_chi_tiet?.ma_nguoi_tim_viec,
      ma_cong_ty: viec.cong_Ty.ma_cong_ty,
      ma_viec: viec.viec_Lam.ma_viec
    }
    this.httpclient.post<API_RESPONSE>(
      ...
      next: (data) => {
        if (data.success) {
          this.danh_sach_lich_su_ung_tuyen = this.danh_sach_lich_su_ung_tuyen.filter(
        item => item.viec_Lam.ma_viec !== viec.viec_Lam.ma_viec);
          this.cd.detectChanges();
        } 
